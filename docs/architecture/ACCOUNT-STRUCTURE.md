...
Domain team infrastructure lives in isolated domain repos (scaffolded from the
platform template repo). Each domain repo manages its own Terraform state
...
4. Platform workflows (GitHub Actions OIDC) use the SSO session; engineers use `aws` CLI commands directly
5. For cross-account operations, assume a cross-account role via `sts:AssumeRole` using the SSO session as source credential
...
6. Trigger the `onboard-domain.yml` workflow to emit `DomainOnboarded` event
